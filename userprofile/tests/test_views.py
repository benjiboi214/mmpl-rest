...
class TestPlayerView(TestCase):
    ...
    def get_view_response(self, action, data=None, authenticate=False):
        ...
        if data:
            request = getattr(factory, action)('/profiles/me', data, format='json')
        else:
            request = getattr(factory, action)('/profiles/me')
        if authenticate:
    ...
    def test_put_updates_profile(self):
        ...
        response = self.get_view_response('put', data={'address': new_address}, authenticate=True)
        self.assertEqual(new_address, response.data['address'])
    
    def test_put_invalid_data_throws_error(self):
        long_address = '''
        Lorem ipsum dolor sit amet, consectetur adipiscing elit. Pellentesque tincidunt neque quis
        ullamcorper blandit. Pellentesque sit amet pharetra dolor. Sed in posuere velit. Curabitur
        eu auctor magna. Blah Blah.o
        '''
        response = self.get_view_response('put', data={'address': long_address}, authenticate=True)
        self.assertEqual(400, response.status_code)
        self.assertIn('no more than 200 characters.', response.data['address'][0])